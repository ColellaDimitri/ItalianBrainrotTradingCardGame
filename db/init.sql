...
-- Sécurité: extension pour index texte rapide (optionnel mais utile)
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'rarete') THEN
...
CREATE TABLE IF NOT EXISTS users (
  ...
  soft_currency    BIGINT NOT NULL DEFAULT 0,      -- monnaie de base gagnée en aventure
  hard_currency    BIGINT NOT NULL DEFAULT 0,      -- optionnel: premium
...
CREATE TABLE IF NOT EXISTS cards (
  ...
  id_capacite    BIGINT NULL REFERENCES capacities(id) ON DELETE SET NULL,
...
VALUES 
('Bluberini Octopusini', 0, 'commun', NULL),
...
('Trippa Troppa Tralala Larila Tung Tung Sahur Boneca Tung Tung Tralalelo Trippi Troppa Crocodina', 0, 'commun', NULL),
...
('Tracotucotulu Delapeladustuz', 0, 'commun', NULL),
...
('Garam Mararam Raraman Mararaman', 0, 'commun', NULL),
...
('Batatoso Peloso, Strambo e Guloso', 0, 'commun', NULL),
...
('Rupipipipipipi Streamimimimimimi', 0, 'commun', NULL),
...
('Los Tralaleritos Dicen Tralala!', 0, 'commun', NULL),
...
('Tukitukitukituki Tukitukitukita', 0, 'commun', NULL),
...
('Luguanluguanlulushijiandaole', 0, 'commun', NULL),
('Malame Amarale', 0, 'commun', NULL),
('Bobrini Cactusini Su Saturno', 0, 'commun', NULL),
...
CREATE TABLE IF NOT EXISTS user_card_collection (
  ...
  position_dans_aventure INTEGER NULL,  -- libre d’usage selon ton gameplay
...
CREATE TABLE IF NOT EXISTS adventure_battles (
  ...
  required_battle_id BIGINT NULL REFERENCES adventure_battles(id) ON DELETE SET NULL,
  reward_soft        INTEGER NOT NULL DEFAULT 0 CHECK (reward_soft >= 0), -- monnaie gagnée
  reward_hard        INTEGER NOT NULL DEFAULT 0 CHECK (reward_hard >= 0), -- optionnel
...
CREATE TABLE IF NOT EXISTS user_adventure_progress (
  ...
  highest_unlocked_battle BIGINT NULL REFERENCES adventure_battles(id) ON DELETE SET NULL,
  last_completed_battle   BIGINT NULL REFERENCES adventure_battles(id) ON DELETE SET NULL,
  updated_at              TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE TRIGGER uap_set_updated_at
...
-- Capacités
INSERT INTO capacities (nom, description) VALUES
  ('Mamma Mia', 'Double la puissance pendant un tour')
ON CONFLICT DO NOTHING;

-- Combats
INSERT INTO adventure_battles (titre, sequence_ordre, required_battle_id, reward_soft, is_boss) VALUES
  ('Ruelle de Napoli', 1, NULL, 50, FALSE),
  ('Pont de Firenze', 2, 1, 75, FALSE),
  ('Duomo Showdown', 3, 2, 150, TRUE)
ON CONFLICT DO NOTHING;
